/**
 * SPIKE — fluxo "Autenticar Procurador" (Termo de Autorização) Serpro Integra Contador.
 * Read-only no fim (Consultar). Modelo: contratante/destinatário = PIPER (cert mTLS),
 * autor/assinante do Termo = AL PISCINAS (cert que assina o XML).
 *
 * Passos:
 *   1. mTLS PIPER → /authenticate → access_token + jwt_token
 *   2. Monta Termo XML (template oficial Serpro) destinatario=PIPER, assinadoPor=AL PISCINAS
 *   3. Assina XMLDSig (RSA-SHA256) com a chave da AL PISCINAS (xml-crypto)
 *   4. POST /Apoiar (AUTENTICAPROCURADOR/ENVIOXMLASSINADO81, dados={"xml":<b64>}) → autenticar_procurador_token
 *   5. POST /Consultar (PGDASD/CONSDECLARACAO13) com header autenticar_procurador_token → dados AL PISCINAS
 *
 * Spec: docs/investigations/SERPRO-INVESTIGACAO.md + lib ref serpro_integra_contador_api.
 * Uso: node scripts/test-serpro-procurador-al-piscinas.mjs
 */
...
async function main() {
  ...
  const apoiar = await req({ host: 'gateway.apiserpro.serpro.gov.br', path: '/integra-contador/v1/Apoiar',
  ...
  const cons = await req({ host: 'gateway.apiserpro.serpro.gov.br', path: '/integra-contador/v1/Consultar',
