...
import ApurarButton from './ApurarButton';
import type { ApuracaoRow } from './page';

export default function SecaoApuracao({
  apuracao,
  competencia,
  podeApurar,
}: {
  ...
  return (
    <div>
      {apuracao ? (
        ...
        <p className="text-sm text-muted-foreground">Sem apuração calculada para esta competência.</p>
      )}
      ...
      {podeApurar && (
        <div className="mt-4">
          {/* Cálculo interno (das notas), idempotente — pode apurar quantas vezes quiser. */}
          <ApurarButton competencia={competencia} recalcular={!!apuracao} />
        </div>
      )}
    </div>
  );
