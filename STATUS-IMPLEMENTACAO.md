...
| Feature | Status | Reusa | Falta |
|---|---|---|---|
| 3.1 Emissor NFS-e integrado | ✅ (2026-05-29) | `src/lib/clients/focus-nfe.ts`, webhook `app/api/webhooks/focus/route.ts`, `notas_fiscais`. Rota `emissao/nfse/page.tsx` + `<EmissaoForm>` + `emitirNotaAction` + `nfse-payload.ts` | emissão real só exercitada em homologação |
