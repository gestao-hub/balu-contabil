...
| `<SaudeEmpresaTab>` | `app/(auth)/configuracoes/SaudeEmpresaTab.tsx` | **Focus 3** — aba "Saúde da empresa" com 5 checks (cidade credenciada Focus, cert presente, cert válido, SERPRO conectada, cadastro Focus funcional). Banner agregado "N/5 ok"; cada check com ícone + hint + ação contextual |
| `<RetryFocusButton>` | `app/(auth)/configuracoes/RetryFocusButton.tsx` | **Focus 3** — client island do botão "Cadastrar na Focus agora" no painel Saúde; dispara `retryFocusEmpresaAction` via `useTransition` + toast |
...
| `updateCompanyAction` + `upsertEmpresaFiscalAction` + `uploadCertificadoAction` + `retryFocusEmpresaAction` | `app/(auth)/configuracoes/actions.ts` | PATCH companies; upsert `empresas_fiscais` (PR 1.4); upload de certificado A1 → Storage + upsert `arquivos_auxiliares` + n8n best-effort (PR 1.6). **Focus 3**: `retryFocusEmpresaAction` reusa `syncEmpresaNaFocus` para o botão "Cadastrar na Focus agora" do painel Saúde |
...
> Funções server-side (não-actions): `getDashboardMetrics` + `getPendingActions` em `lib/dashboard/queries.ts` (**PR 1.1**, `import 'server-only'`); `resolveMunicipioNfse(supabase, municipio, uf)` em `lib/fiscal/municipio-nfse.server.ts` (**PR 1.5**, casa endereço→`municipios_nfse` por nome+UF); `syncEmpresaNaFocus(supabase, companyId)` em `lib/fiscal/focus-empresa-sync.ts` (**Focus 1/2.0** — best-effort: read empresa+regime → POST `/v2/empresas` → grava `companies.focus_*` → GET `/v2/empresas/:id` → snapshot em `empresas_fiscais.focus_*`; nunca lança). Helpers puros: `lib/fiscal/regime.ts` (PR 1.4), `lib/fiscal/municipio-nfse.ts` (PR 1.5), `lib/fiscal/certificado.ts` (PR 1.6), `lib/fiscal/focus-empresa-payload.ts` (**Focus 1** — `buildFocusEmpresaPayload` mapeia input form→payload Focus), `lib/fiscal/saude-empresa.ts` (**Focus 3** — `buildSaudeChecks(state)` retorna os 5 checks; semântica: check 1 = capacidade Focus, check 5 = agregado empresa), `lib/fiscal/municipios-nfsen-nacional.ts` (**Focus 3** — lista hardcoded de aderentes NFSe Nacional; Londrina/PR desde 01/01/2026) e `lib/format/masks.ts`.
...
|---|---|---|
| `focus` (`focus-nfe.ts`) | `consultarCnpj`, `criarEmpresa` (Focus 1, POST `/v2/empresas` na API de revenda — sempre `api.focusnfe.com.br`), `consultarEmpresa(id)` (Focus 2.0, GET `/v2/empresas/:id` pra snapshot), `emitirNfe/Nfce/Nfse`, `consultarStatusNfe/Nfce/Nfse`, `baixarDanfe`, `baixarXmlNfe`, `cancelarNfe/Nfce/Nfse` (valida `justificativa.length ≥ 15`), `generateRef(empresaId)` | retry exponencial em 502/503/504 |
