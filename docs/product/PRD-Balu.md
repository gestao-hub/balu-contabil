...
>
> **Abas hoje (Focus 1+3+4):** Dados da empresa · Regime tributário · **Emissão fiscal** (mesclada: Cert + NFS-e + status Focus) · **Saúde da empresa** (5 checks). As antigas abas "NFS-e" e "Certificado A1" continuam acessíveis pelas URLs `?tab=nfse` e `?tab=certificado` via `TAB_ALIASES` → redirect interno pra `?tab=fiscal`.
>
...
> - **Dados da empresa** (✅ `DadosEmpresaForm` + `updateCompanyAction`): **CNPJ sempre bloqueado**; endereço (rua/número/cidade/estado) obrigatório também na edição. `numero` tem o checkbox **"Sem número"**; CEP/bairro opcionais.
> - **Regime tributário** (✅ `RegimeTributarioForm` + `upsertEmpresaFiscalAction` — PR 1.4): dropdown CRT (`Code_regime_tributario` 1-4); "Faixa de atividade econômica" → `anexo_simples` (visível só se ≠ MEI); `usa_fator_r` (só Anexo III/V); `cnae_principal`. Upsert em `empresas_fiscais` por `empresa_id`.
> - **Emissão fiscal** (✅ `EmissaoFiscalTab` — **Focus 4**): aba única substitui as abas separadas "NFS-e" e "Certificado A1". 3 seções na ordem:
>   - **Certificado A1** (sub-componente `CertificadoForm` — PR 1.6, **ciclo Next sem n8n**): upload `.pfx`/`.p12` + senha → `node-forge` abre o PFX legado (valida a senha de verdade; **bloqueia certificado expirado**) → material de chave (key+cert PEM) **cifrado em repouso (AES-256-GCM, `CERT_ENC_KEY`)** no bucket `company-certificates`; **senha original descartada** (`cert_password = null`) → upsert `arquivos_auxiliares` (storage_key + `cert_not_after`/`cert_subject_cn`/`cert_cnpj`/`cert_fingerprint`) → **best-effort mTLS SERPRO** (`role-type: TERCEIROS`) → cache `certificado_jwt`/`_access_token`/`_token_expiration` em `empresas_fiscais`. Validação ≤1MB + senha obrigatória; senha **write-only**. **Substituir** pede confirmação (`PopupConfirm`). _n8n de certificado **aposentado**._
>   - **NFS-e do município** (sub-componente `NfseForm` + `resolveMunicipioNfse` — PR 1.5): município **resolvido do endereço** (`companies.municipio`+`uf` → `municipios_nfse` por nome+UF, sem IBGE); credenciais conforme `autenticacao` do município (`nfse_usuario_login`/`nfse_senha_login`/`nfse_token_api`); toggle `empresa_fiscal_ativada`/`nfse_habilitada`.
> - **Diagnóstico** (✅ `SaudeEmpresaTab` + `buildSaudeGroups` — **Focus 3** renomeado): visão agregada de "está pronta pra emitir?". 4 grupos com roll-up `erro > pendente > ok`:
>   1. **Cidade credenciada para NFS-e** — _capacidade Focus_ (independente da empresa). Aderente NFSe Nacional (lista hardcoded — `municipios-nfsen-nacional.ts`) → ✓ automático. Senão, consulta `municipios_nfse` legacy.
>   2. **Certificado A1** (2 itens) — Enviado (`arquivos_auxiliares.storage_key`) + Válido (`cert_not_after` vs `now()`, alerta ≤30d).
>   3. **SERPRO conectada** — `empresas_fiscais.certificado_token_expiration` vs `now()` (skew 5min).
>   4. **Cadastro na Focus** (2 itens) — Empresa cadastrada (`focus_token` + `focus_status='ok'`) + Autenticação funcionando (habilita_* + cert presente). Meta line: "Sincronizado em ⟨data⟩" ou alerta de drift quando `companies.updated_at`/`empresas_fiscais.updated_at` > `focus_sync_em`.
>   - **Botão "Sincronizar com Focus"** (`SyncFocusButton` → `syncFocusEmpresaAction` — **Focus 2.1**): adaptativo (POST se sem token; PUT se com), envia **só payload base** (sem cert nem credenciais — esses saem nos seus saves dedicados).

> **Integração Focus NFe (revenda):**
> - **Cadastro automático no `createCompanyAction`** (Focus 1): após criar a empresa no Balu, dispara `POST /v2/empresas` na API de revenda (sempre `api.focusnfe.com.br`; a "homologação" da Focus é por-EMPRESA, aplica-se às emissões, não ao cadastro). Falha **não bloqueia** o cadastro local — fica registrado em `companies.focus_status='erro'` + `focus_last_error`, visível no painel Diagnóstico com botão de retry.
> - **Snapshot** (Focus 2.0): após cada POST/PUT bem-sucedido, `syncEmpresaNaFocus`/`atualizarEmpresaNaFocus` faz `GET /v2/empresas/:id` e popula `empresas_fiscais.focus_*` (id, codigo_municipio, habilita_nfse, habilita_nfsen_producao, habilita_nfsen_homologacao, sync_em). Esse snapshot é fonte de verdade pra "está habilitada pra emitir?" — evita polling.
> - **PUT atualizando** (Focus 2.1): `atualizarEmpresaNaFocus(supabase, companyId, env, extras?)` faz PUT `/v2/empresas/:id` (**id numérico interno**, não CNPJ — empírico + doc oficial). Payload base = dados estruturais (endereço, regime, razão social, IE/IM, flags `habilita_*` decididas por `isAderenteNfsenNacional`). Idempotente. Mesmo helper aceita extras opcionais usados pelos outros gatilhos abaixo.
> - **Cert + senha** (Focus 2.2): no `uploadCertificadoAction`, após cifra local + SERPRO, dispara `atualizarEmpresaNaFocus(..., { certificado: { base64: pfxBytes, senha } })` — único momento em que a senha PFX está em memória natural; depois é descartada. Cert vai como `arquivo_certificado_base64` + `senha_certificado` no mesmo PUT base (sem reempacotar PFX, sem endpoint multipart). Best-effort: erro vira `warning` no return, não bloqueia upload local.
> - **Credenciais prefeitura** (Focus 2.2): no `upsertEmpresaFiscalAction`, quando o patch traz `nfse_usuario_login`+`nfse_senha_login` e o município é legado (`!isAderenteNfsenNacional`), dispara `atualizarEmpresaNaFocus(..., { credenciaisPrefeitura: { login, senha } })`. Mesmo padrão best-effort + warning toast.
> - **Drift detection**: o Diagnóstico compara `max(updated_at locais)` vs `focus_sync_em`. Quando local é mais novo (> 2s), grupo "Cadastro na Focus" vira `pendente` com meta *"Há mudanças não sincronizadas desde …"*.
