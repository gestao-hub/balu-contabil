...
| Feature | Status | Reusa | Falta |
|---|---|---|---|
| 3.1 Emissor NFS-e integrado | ✅ (2026-05-29) | `src/lib/clients/focus-nfe.ts`, webhook `app/api/webhooks/focus/route.ts`, `notas_fiscais`. Rota `emissao/nfse/page.tsx` + `<EmissaoForm>` + `emitirNotaAction` + `nfse-payload.ts`. **Gate de emissão** (2026-06-02): `emitirNotaAction` e `emissao/nfse/page.tsx` verificam `municipios_nfse.status_nfse === 'ativo'` via `codigo_ibge` (substitui toggle manual `empresa_fiscal_ativada`). **As-built 2026-06-09**: o gate `empresa_fiscal_ativada` foi removido TAMBÉM do chooser (`listarTiposEmissaoAction`) e dos validadores NF-e/NFC-e — era órfão (nada o setava `true`) e travava toda empresa; habilitação passou a ser só `focus_habilita_*`. E `companies.codigo_municipio` (IBGE) agora é resolvido pelo CEP (ViaCEP `ibge`, helper `ibge-por-cep.ts`) no cadastro e no "Atualizar da Receita" — sem ele a NFS-e travava em "Município sem código IBGE". | emissão real só exercitada em homologação |
| 3.1b Emissor NF-e + NFC-e (multi-tipo) | ✅ código (2026-05-30) · ❌ habilitação Focus | **Emissão multi-tipo**: tela de escolha `emissao/page.tsx` (3 cards gated por flag), forms `emissao/{nfe,nfce}/`, builders `lib/fiscal/{nfe,nfce}-payload.ts` (+testes), `emitirNfeAction`/`emitirNfceAction`, `ItensField` (catálogo `aux_produtos` + criação inline), produtos compartilhados nfe/nfce. Migrations `0012` (flags `focus_habilita_nfe/nfce` + AL Piscinas) e `0013` (cria `aux_produtos`). Spec `docs/superpowers/specs/2026-05-30-emissao-multitipo-nfe-nfce-design.md`. **As-built 2026-06-09**: `focus_habilita_nfe/nfce` agora são populados pelo `snapshotFocusEmpresa` (copia `habilita_nfe/nfce` do `GET /v2/empresas`; antes ficavam eternamente `false`) | **Bloqueio externo**: empresa não habilitada na Focus → `empresa_nao_habilitada` (mesmo padrão do gap de NFS-e prod). Verificado em homologação 2026-05-30: payload aceito/processado, Focus responde até a checagem de atividade. Impostos por item com defaults fixos (refino fiscal pendente) |
| 3.1c Criação via modal (emissão + nota manual) | ✅ (2026-06-08) | **Refator modal-only**: rotas `/notas_fiscais/emissao*` e `/manual` **removidas**; UI em `_nova-nota/`. Dois modais `<dialog>`: `EmitirNotaDialog` (emissão) e `NotaManualDialog` (manual), ambos multi-step (chooser de 3 cards → form; não habilitados desativados via `listarTiposEmissaoAction`). Forms `EmissaoForm`/`NfeForm`/`NfceForm` reusados via prop `modo: 'emissao'\|'manual'` (manual = idêntico + Número/Data, grava `lancarNotaManualAction` — união por tipo: `status='lancada'`, `origem='manual'`, `cnae` p/ NFS-e, `numero_nf`, valida ownership do cliente). Actions `prepararEmissaoAction(tipo)` (com guards) e `prepararNotaManualAction(tipo)` (sem guards). Dropdown "Nova nota" no header; data manual em `dd-mm-aaaa` (`DataEmissaoBR`); `ClienteCombobox` + seletor de empresa flutuantes. Spec `docs/superpowers/specs/2026-06-08-emissao-modal-design.md` | pós-sucesso fecha+refresh; produção segue só homologação |
