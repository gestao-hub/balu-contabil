...
| **1.2** | **DASN-SIMEI anual (MEI)** | ✨ | V1 §7.1 promete entrega da declaração anual do MEI. Zero código hoje. **API investigada (2026-06-06):** `DASNSIMEI` — `TRANSDECLARACAO151` (entregar), `CONSULTIMADECREC152` (consultar), `GERARDASEXCESSO153` (DAS excesso, ⚠️ não contratável). Payload mínimo (receitaComercio+receitaServico+indicadorEmpregado), prazo **31/05**, **sem dry-run**, MAED+excesso na resposta. MVP = transmitir+consultar; excesso adiado. Doc `docs/investigations/DASN-SIMEI.md` + spec `2026-06-06-dasn-simei-design.md`. Único bloqueio: smoke precisa de e-CNPJ MEI real (puro+parser+UI codáveis já). | M | 1.1 |
