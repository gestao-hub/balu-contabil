...
**Progresso P1 (2026-06-06):** ✅ P1.1 (Declarações no `/impostos`). 🟡 **P1.2** (DASN-SIMEI) — camada pura + consulta/histórico + fallback manual feitos; **transmissão bloqueada na SERPRO** (`TRANSDECLARACAO151` não disponível p/ contratação). Ver `docs/investigations/DASN-SIMEI.md`.

**Próximo candidato:** P1.3 (consulta DAS/situação MEI mensal), P1.4/1.5 (versionar tabela / anualização RBT12), ou PGDAS-D Fase 2 (transmit real — spec `2026-06-06-pgdasd-transmissao-fase2-design.md`).
...
| **1.2** | **DASN-SIMEI anual (MEI)** | ✨ | V1 §7.1 promete entrega da declaração anual do MEI. **API investigada (2026-06-06):** `DASNSIMEI` — `TRANSDECLARACAO151` (entregar), `CONSULTIMADECREC152` (consultar), `GERARDASEXCESSO153` (DAS excesso). Payload mínimo (receitaComercio+receitaServico+indicadorEmpregado), prazo **31/05**, **sem dry-run**, MAED+excesso na resposta. 🚧 **BLOQUEIO EXTERNO:** o `TRANSDECLARACAO151` (entregar) está marcado "AINDA NÃO DISPONÍVEL PARA CONTRATAÇÃO / pode sofrer alterações" e dá 101507 no Trial → **não dá pra transmitir hoje** (nem prod). **FEITO** (commits `a0531f1`+`1a091a9`): camada pura (builder+receita+parser, TDD), **consulta/histórico** (`CONSULTIMADECREC152` + ação + seção MEI no `/impostos`) e **fallback manual** (link pro portal `dasnsimei.app`). **Falta (bloqueado na SERPRO):** transmissão real (`transmitirDasnSimei` + gate + PDFs). Doc `docs/investigations/DASN-SIMEI.md` + spec `2026-06-06-dasn-simei-design.md`. | M | 1.1 + liberação SERPRO |
