...
| Documentos fiscais (notas emitidas, guias DAS, apurações de imposto, declarações PGDAS-D/DASN-SIMEI) | `notas_fiscais`, `guias_fiscais`, `apuracoes_fiscais`, `declaracoes_fiscais` | Emissão fiscal, apuração e declaração de tributos do Simples Nacional | Art. 7º, II (obrigação legal/regulatória — guarda fiscal) | ~5 anos (prazo da legislação fiscal/decadencial), independente do estado da conta. Na exclusão da conta, as linhas de origem (`companies`, `clientes`) são anonimizadas, mas o **snapshot do documento emitido** (`notas_fiscais.payload_focusnfe`) **retém intencionalmente** os dados como emitidos (razão social/CNPJ/endereço do emitente e do destinatário na data da emissão) — a nota fiscal deve refletir o que foi de fato emitido (Art. 7º, II) | Titular da empresa; dados do cliente destinatário da nota, quando aplicável |
| Abertura/alteração de empresa (dados do sócio: nome completo, CPF, RG, nome da mãe, data de nascimento, estado civil, nacionalidade, endereço residencial) | `abertura_empresas`, `abertura_alteracoes` | Instrução do processo de abertura/alteração de empresa nos órgãos competentes | Art. 7º, I (execução do serviço solicitado) | Dado operacional (não é documento fiscal a reter): **apagado** na exclusão da conta do titular (minimização) | Titular / sócio da empresa (dado de alto impacto: RG, nome da mãe) |
