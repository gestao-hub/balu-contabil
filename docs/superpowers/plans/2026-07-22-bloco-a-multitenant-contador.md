...
- [ ] **Step 3: Menu + layout** — `(auth)/layout.tsx`: o normalize atual (`toLowerCase`) já produzirá `'adminbalu'`; ampliar o tipo do `userRole` para `'empresa' | 'contador' | 'adminbalu'` aqui e no `MenuLateral`. No `MenuLateral.NAV` adicionar `{ href: '/admin/contabilidades', label: 'Admin', Icon: ShieldCheck, roles: ['adminbalu'] }` (import `ShieldCheck` de `lucide-react`). Atenção: usuários AdminBalu sem empresa cairiam no `redirect('/onboarding')` do layout — condicionar: `if (!profile?.current_company && !['adminbalu', 'contador'].includes(normalizedRole)) redirect('/onboarding')`. **[EMENDA 22/07, descoberta na Task 10]** o papel `contador` também precisa da isenção: um contador recém-cadastrado não tem empresa e era mandado ao `/onboarding` antes de conseguir chegar em `/contador/cadastro` (o onboarding só sabe criar `companies`, não `contabilidades`).
