...
|---|---|
| `app/supabase/migrations/0004_notas_emissao_cancelamento.sql` (novo) | +11 colunas aditivas em `notas_fiscais`. |
| `app/src/types/database.ts` (modificar) | Tipos das novas colunas. |
| `app/src/lib/fiscal/notas-tipo.ts` (novo) | Puro: `assertTipoDoc` + `validarJustificativa`. Testável. |
| `app/src/lib/fiscal/notas-tipo.test.ts` (novo) | Testes do módulo puro. |
| `app/src/components/PopupConfirm.tsx` (modificar) | Slot opcional `children` (retrocompatível). |
| `app/src/app/(auth)/notas_fiscais/actions.ts` (modificar) | `cancelarNotaAction(id, justificativa)`. |
| `app/src/app/(auth)/notas_fiscais/[id]/download/route.ts` (novo) | Proxy de download XML/PDF. |
| `app/src/app/(auth)/notas_fiscais/[id]/CancelarButton.tsx` (novo, client) | Confirmação destrutiva + justificativa. |
| `app/src/app/(auth)/notas_fiscais/[id]/page.tsx` (substituir stub) | Detalhe (server). |

Comandos rodam de `app/`. Branch: `feat/pr-1.3-detalhe-cancelamento` (NÃO trocar de branch).
...
**Files:**
- Create: `app/supabase/migrations/0004_notas_emissao_cancelamento.sql`
- Modify: `app/src/types/database.ts` (bloco `notas_fiscais`, ~linhas 132-142)
...
In `app/src/types/database.ts`, replace the `notas_fiscais` block:
```ts
  notas_fiscais: {
    id: string;
    company_id: string;
...
```bash
git add app/supabase/migrations/0004_notas_emissao_cancelamento.sql app/src/types/database.ts
...
**Files:**
- Create: `app/src/lib/fiscal/notas-tipo.ts`
- Test: `app/src/lib/fiscal/notas-tipo.test.ts`

- [ ] **Step 1: Escrever o teste que falha**

Create `app/src/lib/fiscal/notas-tipo.test.ts`:
```ts
...
Run: `cd app && npx vitest run src/lib/fiscal/notas-tipo.test.ts`
...
```bash
git add app/src/lib/fiscal/notas-tipo.ts app/src/lib/fiscal/notas-tipo.test.ts
...
**Files:**
- Modify: `app/src/components/PopupConfirm.tsx`
...
In `app/src/components/PopupConfirm.tsx`, in `PopupConfirmProps`, add after the `busy?: boolean;` line:
```ts
...
```bash
git add app/src/components/PopupConfirm.tsx
...
**Files:**
- Modify: `app/src/app/(auth)/notas_fiscais/actions.ts` (adicionar imports + a action)

- [ ] **Step 1: Adicionar imports no topo**

In `app/src/app/(auth)/notas_fiscais/actions.ts`, after the existing `import { createServerClient } from '@/lib/supabase/server';`, add:
```ts
...
Append to `app/src/app/(auth)/notas_fiscais/actions.ts`:
```ts
...
**Files:**
- Create: `app/src/app/(auth)/notas_fiscais/[id]/download/route.ts`
...
```bash
git add "app/src/app/(auth)/notas_fiscais/[id]/download/route.ts"
...
**Files:**
- Create: `app/src/app/(auth)/notas_fiscais/[id]/CancelarButton.tsx`
...
```bash
git add "app/src/app/(auth)/notas_fiscais/[id]/CancelarButton.tsx"
...
**Files:**
- Modify (substituir conteúdo): `app/src/app/(auth)/notas_fiscais/[id]/page.tsx`

- [ ] **Step 1: Substituir o stub pelo detalhe**

Replace the entire content of `app/src/app/(auth)/notas_fiscais/[id]/page.tsx` with:
```tsx
...
```bash
git add "app/src/app/(auth)/notas_fiscais/[id]/page.tsx"
