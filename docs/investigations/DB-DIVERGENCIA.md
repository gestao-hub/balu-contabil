...
**Passo 3** — Day 3: reescrever o wizard de apuração/DAS para o formato real (`apuracoes_fiscais`/`guias_fiscais` do motor n8n); a receita vem de `notas_fiscais` (`receitas_fiscais` descontinuada — ver Passo 4). **Decisão (2026-06-05): RESOLVIDO** — `declaracoes_fiscais` criada de forma aditiva pela `0025` na convenção real (`company_id`/`competencia_referencia`/`owner_user_id`), NÃO na convenção idealizada da 0001. Ver spec `2026-06-05-declaracoes-impostos-design.md`.
