...
> - **Certificado A1** (✅ `CertificadoForm` + `uploadCertificadoAction` — PR 1.6): upload `.pfx`/`.p12` + senha → Supabase Storage (bucket `company-certificates`, service_role) → upsert `arquivos_auxiliares` (`supabase_file_path`, `cert_password`; senha **write-only**, nunca volta ao client) → notifica `n8n.uploadCertificado` (**best-effort**: falha do n8n não perde o upload, vira aviso). Status read-only ("Certificado enviado em {data}"). Validação `.pfx`/`.p12` ≤1MB + senha obrigatória; nome sanitizado (anti path-traversal). **Substituir** um certificado já existente pede **confirmação** (`PopupConfirm` destrutivo) avisando do impacto na emissão/fluxos antes de subir; o primeiro envio sobe direto. Substituir por arquivo de nome diferente **remove o objeto antigo** do bucket. _Carry-forward: `cert_password` em texto puro (ver memória); campos `certificado_*` em `empresas_fiscais` são preenchidos pelo workflow n8n._
