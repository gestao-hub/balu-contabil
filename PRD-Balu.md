...
>   - **Status na Focus** (read-only): dot verde "Cadastrada · token presente · última verificação ⟨data⟩" / dot vermelho "Erro" / dot cinza "Não cadastrada". Ações de retry ficam na aba "Saúde da empresa". (PUT enriquecendo cert + senha prefeitura na Focus = **Focus 2.1**, em backlog.)
> - **Saúde da empresa** (✅ `SaudeEmpresaTab` + `buildSaudeChecks` — **Focus 3**): visão agregada de "está pronta pra emitir?". 5 checks com banner "N/5 ok":
...
>   2. **Certificado A1 enviado** — `arquivos_auxiliares.storage_key`.
>   3. **Certificado A1 válido** — `cert_not_after` vs `now()` (alerta amarelo ≤30d, erro se expirado).
>   4. **SERPRO conectada** — `empresas_fiscais.certificado_token_expiration` vs `now()` (skew 5min).
>   5. **Cadastro na Focus funcional** — _agregado por-empresa_: `companies.focus_token` + alguma `focus_habilita_*=true` no snapshot + cert presente.
>   - Botão "Cadastrar na Focus agora" (`RetryFocusButton` → `retryFocusEmpresaAction`) reusa `syncEmpresaNaFocus` para corrigir cadastros pendentes/com erro.

> **Integração Focus NFe (revenda):**
> - **Cadastro automático no `createCompanyAction`** (Focus 1): após criar a empresa no Balu, dispara `POST /v2/empresas` na API de revenda (sempre `api.focusnfe.com.br`; a "homologação" da Focus é por-EMPRESA, aplica-se às emissões, não ao cadastro). Falha **não bloqueia** o cadastro local — fica registrado em `companies.focus_status='erro'` + `focus_last_error`, visível no painel Saúde com botão de retry.
> - **Snapshot** (Focus 2.0): após cada POST/PUT bem-sucedido, `syncEmpresaNaFocus` faz `GET /v2/empresas/:id` e popula `empresas_fiscais.focus_*` (id, codigo_municipio, habilita_nfse, habilita_nfsen_producao, habilita_nfsen_homologacao, sync_em). Esse snapshot é fonte de verdade pra "está habilitada pra emitir?" — evita polling.
> - **PUT enriquecendo** (Focus 2.1, **backlog**): no upload do certificado, reaproveitará a senha digitada (descartada após o PUT) e mandará pra Focus junto com flags `habilita_*` + credenciais da prefeitura. Em breve.
