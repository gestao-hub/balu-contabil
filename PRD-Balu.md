...
> - **NFS-e** (✅ `NfseForm` + `resolveMunicipioNfse` — PR 1.5): o município é **resolvido do endereço** (`companies.municipio`+`uf` → `municipios_nfse` por nome+UF, sem IBGE); bloco read-only derivado (provedor/autenticação/cancelamento/flags); credenciais conforme `autenticacao` do município (`nfse_usuario_login`/`nfse_senha_login`/`nfse_token_api`); certificado → aba própria (PR 1.6); toggle `empresa_fiscal_ativada`/`nfse_habilitada`. Persiste em `empresas_fiscais` (grava `municipio_id`; flags lidas por join, não copiadas).
> - **Certificado A1** (✅ `CertificadoForm` + `uploadCertificadoAction` — PR 1.6; **ciclo assumido pelo Next, sem n8n** — ver `docs/superpowers/specs/2026-05-27-certificado-next-sem-n8n-design.md`): upload `.pfx`/`.p12` + senha → `node-forge` abre o PFX legado (valida a senha de verdade; **bloqueia certificado expirado**) e extrai metadados → o material de chave (key+cert PEM) é **cifrado em repouso (AES-256-GCM, `CERT_ENC_KEY`)** como blob `.enc` no bucket `company-certificates` (service_role) e **a senha do certificado é descartada** (`cert_password = null`) → upsert `arquivos_auxiliares` (`supabase_file_path`/`storage_key` + `cert_not_after`/`cert_subject_cn`/`cert_cnpj`/`cert_fingerprint`) → **best-effort**: autentica **mTLS na SERPRO** (autenticar procurador, `role-type: TERCEIROS`) e cacheia `certificado_jwt`/`_access_token`/`_token_expiration` em `empresas_fiscais` (falha não perde o upload, vira aviso). Status read-only ("Certificado enviado em {data}. Válido até {data}."). Validação `.pfx`/`.p12` ≤1MB + senha obrigatória; senha **write-only**, nunca volta ao client. **Substituir** pede **confirmação** (`PopupConfirm` destrutivo); re-upload sobrescreve o mesmo `.enc` (path estável por `unique_id_bubble`). _n8n de certificado (`upload-certificado`/`post-autenticacao`) **aposentado** (validado contra cert real + SERPRO prod). Downstream: consumir o `certificado_jwt` nas chamadas Declarar/Emitir/Consultar do `serpro.ts`._
